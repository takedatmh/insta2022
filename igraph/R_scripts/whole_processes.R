...
##AIC Binomial Logistic Regression backward Analysis
result.d = read.csv("/Users/amazon/app/01.eclipseWorkspaces/luna_soot/soot/tmp/igraph/csv/result/resultDf_binominal.csv", header=T, s=",")
result_tmp = result.d[,4:14]
result_aic_backward = step(glm(teaching_c~. , data=result_tmp, family=binomial), direction="backward")
plot(result_tmp[,1],xlim=c(1,118), ylim=c(0,1), ylab = "", col="red", pch=8, main = "")
par(new=T)
plot(fitted(result_aic_backward),xlim=c(1,118), ylim=c(0,1), ylab="T/F", col="blue", pch=1, main="Prediction by Logistic Regression & AIC Backword")
