...
System.out.println("=_=; " + resultPathMap.get(key));
	...
	public static Map<Integer, List<String>> searchPath(UnitGraph graph,
		...
		/*
		 * 0 currentNodeに対してseenの該当Nodeをtrueにする 
		 * 　且、終端ノードでないことを確認する。終端ノードなら0.1, 終端ノードでない場合1.へ移動。
		 * 　0.1.終端ノード & todoにtrueでないnodeがある場合：終端ノードをvisitNodeListに加えて、Mapに
		 *       visitNodeListを格納
		 *    0.1.1.visitNodeListの最終インデックスから左にインデックスを下げて行きながらseenの中でfalse
		 *          になっているノードをchildに持つノードを逆探索する 　
		 *    0.1.2.逆探索してseen falseのノードを子供として持つノードをcurrentノードとして引数指定で４に戻る。
		 *          その際、visitNodeListをコピーして、開始インデックスからvisitNodeListを上書きして行く。 　
		 *   0.2.終端ノード & todoがすべてtrueの場合：終端ノードに対してsennをtrue, NodeListに加えて、
		 *       MapにnodeListを格納し、終了。
		 * 1.CurrentNodeの子供たちを検索する。(Succs)
		 * ２.visitNodeListに現在ノード(子供の親ノード)を格納する。 
		 * ３.todoに子供たちを入れる。
		 * ４.todoから子供を一人取り出して、todoから消す。 
		 * ５.取り出した子供がseenの中でtrueになってないか調べる
		 * 　5.1.trueだったら４に戻って別な子供を取り出す 
		 *    5.1.1.こどもがいなくなったら終わり 　
		 *   5.2.falseだったら
		 *     5.2.1.todoから取り出したノードを指定して1に戻る
		 */
